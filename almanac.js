...
function bh(x) {
x1=-(34/60+9/3600-HP25/3600+SD25/3600);
if ( x<x1) x2=" (Below Horizon) ";
else x2="";
return x2; }
function na(x) {
if (isNaN(x)) x2="Not Possible ";
else x2=x;
return x2; }
...
z=Math.floor(Math.abs(x)/360);
w= (Math.abs(x)/360-z)*360;
u=sn*w;
if ( u>180) u=u-360;
else if ( u<-180) u=360+u;
else u = u;
return u;	}
function loo3(x) {
with (Math)
...
with(Math)	{
	...
	var GHAsec = round(3600000*(x-GHAdeg-GHAmin/60))/1000;
...
function OutDec(x){
with(Math){
	if (x<0)  {var signDEC=-1; var name="-";}
	else  {var signDEC=1; var name="";}
	var DEC = abs(x);
	var DECdeg = floor(DEC);
	var DECmin = floor(60*(DEC-DECdeg));	
	var DECsec = round(3600*(DEC-DECdeg-DECmin/60))/1;
...
function time(x){
with(Math){
	if (x>24) { var x = x-24; var name = "T";}
	else if ( x<0 ) { var x= x+24; var name ="Y";}
	else { var x=x; var name ="";}
	var DEC = abs(x);
	var DECdeg = floor(DEC);
	var DECmin = floor(60*(DEC-DECdeg));	
	var DECsec = round(3600*(DEC-DECdeg-DECmin/60))/1;
	...
	if (DECsec<10) DECsec="0"+DECsec;
	if ( isNaN(x) ) name2= " نہ ممکن";
	if ( isFinite(x)) name2 = name+DECdeg+":"+DECmin+":"+DECsec;
	return name2;
	}}
function tim(x){
with(Math){
	...
	var DEC = abs(x);
	var DECdeg = floor(DEC);
	var DECmin = round(60*(DEC-DECdeg));	
...
function disdt(y,m,d){
with(Math){
var options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
var today  = new Date(y,m-1,d);
dt= today.toLocaleDateString("ur-US",options);}
return dt;}
function eot(a){//EOT
with(Math){
t= norf(a,loo(15*24*dayfraction))/15;
if (t<0) t +=12;
else t -=12;}
return t;}
function namaz(a,t,m,z,e,f){
with(Math){
		hr = 12 - toolb/15 + e + f * acos((cosd(z) - sind (arzb) * sind (m ))/(cosd(arzb) * cosd(m)))/15/dtr;
		}}
function nmz(z,e,m,f){ with(Math) 
		hr = 12 - toolb/15 + e + f * acos((cosd(z) - sind (arzb) * sind (m ))/(cosd(arzb) * cosd(m)))/15/dtr;
		return hr;} 
function ref11(t,p,z){
{with(Math)
var ref= p/1010*283/(273+t)*1.02*tand(z-10.3/(95.11-z))/60;}
         return ref;}
function ref12(t,p,z){
{with(Math)
var ref= refa2r = p/1010 * 283/(273 + t) * tand ( z- 7.31/ (94.4 - z))/60;       }
         return ref;}
       function d360 (f,d) {
       {with(Math)
       if (f>0)  {var  dc = 90 - d ;}
      ...
      function GHA(GHA){
      {with (Math)
      if ( GHA > 180 ) GHA = GHA -360;}
      return GHA;}
       function norf(b,d){
       {with (Math)
       var f = d-b;
       if (f<-180) var f= 360- Math.abs(f);
       else if ( f > 180) var f =f-360;
       else      var f = f;}
       return +f;}
   function dirp2p2(a,b,c,d,e){
       {with (Math)
             b= b*e;
             if (b>180) b = b-360;
             if (b < -180 ) b = b+360;
             ...
             if (d < -180 ) d = d+360;
       f= -norf(b,d);
      var dir = Math.atan2(sind(f),(tand(a) * cosd (c) - sind (c)* cosd (f )))/dtr;
     if (dir<0)  {var dirs = 360 + dir ;}
     else {var dirs = 0 + dir;}
       return dirs;}}
       function dirp2p(a,b,c,d,e){
       {with (Math)
      b= b*e;
      if (b>180) b = b-360;
      if (b < -180 ) b = b+360;
      ...
      if (d < -180 ) d = d+360;
       f= norf(b,d);
       var dir = Math.atan((tand(a) * cosd (c) - sind (c)* cosd (f ))/ sind (f))/dtr;
       if (f<0)  {var dirs = 90 + dir ;}
       else {var dirs = 270 + dir;}}
  //     if(dirs==360) dirs=0;
       return dirs;}
       function disp2p(a,b,c,d,e){
       {with (Math)
       b= b*e;
       if (b>180) b = b-360;
       if (b < -180 ) b = b+360;
       ...
       if (d < -180 ) d = d+360;
       f= norf(b,d);
       var dis = Math.acos(cosd(f)*cosd(a)*cosd(c)+sind(a)*sind(c))/dtr*6378.137/180*Math.PI;
       if (dis<1) {dst = ""+ Math.round(100000*dis)/100000*1000 + " m";}
       else { dst = ""+ Math.round(10000*dis)/10000+ " km";}}
       return dst;}
function elv(a,b,c,d,e){
    {with (Math)
    b= b*e;
    if (b>180) b = b-360;
    if (b < -180 ) b = b+360;
    ...
    if (d < -180 ) d = d+360;
    f= norf(b,d);    
     hight = Math.asin (sind(c) * sind (a) + cosd (c) * cosd(a) * cosd (f))/dtr ;}
     return hight}
       function fdirt (Z,DEC,dir,arz,f)
       {with(Math)
       { if (dir <= 180) { var dir = dir-90;}
       else { var dir = dir-270;}
       dirf= Z + 2*atan((cosd(DEC)*(tand(dir)+f*pow((pow(tand(dir),2)+pow(sind(arz),2)-pow(cosd(arz),2)*pow(tand(DEC),2)),0.5)))/sind(arz+DEC))/dtr/15;}
       return +dirf;}
function fasal(GHA,toolb){
	{with (Math)
	if (toolb >180) toolb = toolb-360;
	if (toolb < -180 ) toolb = toolb+360;
	fasal = toolb+GHA;
	if (fasal>180) fasal = fasal-360;}
	return fasal;}
	function zloop(a,b,c,d,e,f)	{
	with(Math)	{
	if(Math.abs(a-b)>0.000999/3600) {
	i = 1;
	for (; Math.abs(a-b)>0.000999/3600   && i<92  ; ) {
	dataF(year26,month26,day26+f,b,0,0,Tdelta);
	a=b;
	b = c-toolb/15+ EOT25;	
	i++; 	}
	hr41 = b;}
...
function fdtt(dir,x) {
	...
	b = 12 + EOT25 - toolb/15 + 2*Math.atan((cosd(DEC25)*(tand(dir)+x*pow((pow(tand(dir),2)+pow(sind(arzb),2)-pow(cosd(arzb),2)*pow(tand(DEC25),2)),0.5)))/sind(arzb+DEC25))/dtr/15;}
	if ( Math.abs(a-b) > df) {
	i = 1;
	for (; Math.abs(a-b)>df && i<92  ; ) {
	dataF(year26,month26,day26,b,0,0,Tdelta);
	a=b;
	b = 12 + EOT25 - toolb/15 + 2*Math.atan((cosd(DEC25)*(tand(dir)+x*pow((pow(tand(dir),2)+pow(sind(arzb),2)-pow(cosd(arzb),2)*pow(tand(DEC25),2)),0.5)))/sind(arzb+DEC25))/dtr/15;
	i++;}
	hr7 = b+st1;}
...
function dataF(y11,m11,d11,h11,m12,s11,d12) {{
with(document)	{
		yy26=year = y11;
		mm26=month = m11;
		dd26=day = d11;
...
//	seot = eot(sGHA);
	...
	k = 100*(1-cosd(i))/2;
	k = round(1000000000*k)/1000000000;
	}
}
...
mea=betaM;
Lm=lambdaMapp;
//Ls=Math.acos(cosd(RAsun)*cosd(DECsun))/dtr;
//if (RAsun>180) Ls=360-Ls;
//Ls=Math.acos(cosd(RAsun)*cosd(DECsun)/cosd(betas))/dtr;
//a53=Math.abs(norf(Ls,lambda));
//sea=Math.atan(tand(DECsun)/sind(RAsun))/dtr;
//sea1=Math.asin(sind(DECsun)*cosd(sea)-cosd(DECsun)*sind(sea)*sind(RAsun))/dtr;
//mea=Math.asin(sind(DECmoon)*cosd(sea)-cosd(DECmoon)*sind(sea)*sind(mtqwm))/dtr;
//Lm=Math.acos(cosd(mtqwm)*cosd(DECmoon)/cosd(mea))/dtr;
//if (mtqwm>180) Lm=360-Lm;
k2 = lambdaMapp-lambda;
if(k2>=0) k3="+";
else k3="-";
k4=100*(1-cosd(k2))/2;
kk2=k3+""+k4+" %";
kk3=k3+""+100*(1-cosd(LDist))/2+" %";
//Weekday();
{
JD0h += 1.5;
